    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddHealthChecks()
                .AddDbContextCheck<WritingContext>();

            services.AddDbContext<WritingContext>(options =>
                options.UseSqlServer(
                    Configuration.GetConnectionString("DefaultConnection"),
                    b => b.MigrationsAssembly(typeof(WritingContext).Assembly.FullName)));
                       
            services.AddScoped<IApplicationDbContext>(provider => provider.GetService<WritingContext>());

            var assembly = AppDomain.CurrentDomain.Load("Restaurante.Application");
